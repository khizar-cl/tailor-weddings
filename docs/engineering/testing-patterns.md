...
Integration tests run against a real PostgreSQL database. The test command auto-starts the test database, but you can also start it manually:

```bash
pnpm infra:test:up
```

This starts a PostgreSQL instance on port **5434** (separate from the dev DB on 5433) with database `tailorweddings_test`. Set the corresponding env var:

```
TEST_DATABASE_URL=postgresql://postgres:postgres@localhost:5434/tailorweddings_test
```

The setup is:

1. **Global setup** (`apps/server/vitest.global-setup.ts`) validates that `TEST_DATABASE_URL` contains `"test"` (safety check), then syncs the schema with `drizzle-kit push --force`. This substring check is intentionally simple — CI always uses `tailorweddings_test` as the database name. For extra safety in local development, use a database name like `tailorweddings_test` that makes the intent obvious.
...
CI configures a PostgreSQL 17 service and sets `TEST_DATABASE_URL=postgresql://postgres:postgres@localhost:5432/tailorweddings_test`.
