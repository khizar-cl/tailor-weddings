...
- `tailorweddings-api-prod` — Express API (Docker)
- `tailorweddings-worker-prod` — BullMQ worker (uncomment only if you need scheduled and background tasks)
- `tailorweddings-redis-prod` — Redis cache
- `tailorweddings-postgres-prod` — PostgreSQL database

After uncommenting, commit and push to `develop`, then go to Render dashboard → **Blueprints** → your Blueprint → **Sync**.

`tailorweddings-api-prod`'s initial deploy will fail because secrets aren't set yet — this is expected. Don't wait for it (takes ~5 min); proceed to set secrets below. The worker might fail too if it's using env vars.
...
Get the **External Database URL** from Render → `tailorweddings-postgres-prod` → **Connections** → **External Database URL**. Always append `?sslmode=require`.
...
Get your API URL from Render → `tailorweddings-api-prod` (or whatever you renamed it to) → it's displayed at the top of the service page, right under the service name. Append `/health` to that URL and use it in place of the example below if yours differs.

```bash
curl https://tailorweddings-api-prod.onrender.com/health
```
...
- `NEXT_PUBLIC_SERVER_URL` = `https://tailorweddings-api-prod.onrender.com` (get your actual URL from Render → `tailorweddings-api-prod` → displayed at the top of the service page, right under the service name)
...
Then trigger a **manual redeploy** on the Render API so it picks up the new value: Render → `tailorweddings-api-prod` → **Manual Deploy** → **Deploy latest commit**.
...
Go to GitHub repo → **Settings → Environments → New environment** → name it `production`. Add the secrets listed in [`ENV_VARS.md`](ENV_VARS.md) under **GitHub — Environment Secrets**, using the `tailorweddings-*-prod` Render services and the Vercel production deploy hook created in Step 2.
