...
| Variable | Required | How to get it |
|---|---|---|
| `NEXT_PUBLIC_SERVER_URL` | Yes | The Render API service URL for this env (e.g. `https://tailorweddings-api-staging.onrender.com`). Get it from Render → open the API service (`tailorweddings-api-staging` / `tailorweddings-api-prod` or whatever you renamed it to) → the URL is displayed at the top of the service page, right under the service name. |
