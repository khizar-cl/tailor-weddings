...
2. Rename services, databases, and Redis with the new suffix (e.g. `tailorweddings-api-preview`, `tailorweddings-worker-preview`)
...
`tailorweddings-api-preview`'s initial deploy will fail because secrets aren't set yet — this is expected. Don't wait for it (takes ~5 min); proceed to set secrets below. The worker might fail too if it's using env vars.
...
Get the **External Database URL** from Render → `tailorweddings-postgres-<env>` → **Connections** → **External Database URL**. Always append `?sslmode=require`.
