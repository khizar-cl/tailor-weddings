...
module "render" {
  ...
  // Open external Postgres access so the GitHub Actions migration workflow
  // (ephemeral runner IPs) can reach the DB. Connection is still TLS-encrypted
  // (sslmode=require) and the password is a strong random string.
  postgres_ip_allow_list = [
    {
      cidr_block  = "0.0.0.0/0"
      description = "open external access for GitHub Actions migration workflow"
    },
  ]
}
